...
#define LOG_TAG "AOVHook.WVMemoryHook"
...
namespace AOVHook
{
    ...
    static void HackDlExtInfoOnDemand(void* extinfo) {
        ...
        if ((extFlags & ANDROID_DLEXT_RESERVED_ADDRESS) != 0) {
            ...
            extFlags = (extFlags & (~ANDROID_DLEXT_RESERVED_ADDRESS)) | ANDROID_DLEXT_RESERVED_ADDRESS_HINT;  // 替换 为 HINT
    ...
    static jmethodID GetMethodIDByMetaReflect(JNIEnv* env,
        ...
        auto cleaner = MakeScopedCleaner([env]() {
...
//        int refreshCode =  xh_core_refresh(1);

//        LOGE(LOG_TAG, "xh_core_refresh directly , return code is :%d ",refreshCode );
        bool result = false;
        {
